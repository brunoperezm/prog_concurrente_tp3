	...
	public void run() {
		while (!interrupted()) {
			...
			if (mCPUNumber.equals(TasksManager.CPUNumber.CPU1)) {
				if(mMonitor.fireTransitions(Utils.getSingleTransition(PN.Transitions.WAKE_UP_1))) {
					if (mMonitor.fireTransitions(Utils.getSingleTransition(PN.Transitions.RETURN_PENDING_TASK_1))) {
						try {
							Thread.sleep(50);
				...
				if(mMonitor.fireTransitions(Utils.getSingleTransition(PN.Transitions.WAKE_UP_2))) {
					if (mMonitor.fireTransitions(Utils.getSingleTransition(PN.Transitions.RETURN_PENDING_TASK_2))) {
						try {
							Thread.sleep(50);
						} catch (InterruptedException e) {
							e.printStackTrace();
